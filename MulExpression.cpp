...
#include "MulExpression.h"	// Class definition
...
// Definitions
#define MULPRECISION	30	// Taken as the precision of a multiplication result
...
//						=======================
ExprResult				MulExpression::Evaluate
//						======================= 
(
	Context&			context
) const
{
	...
	if ( left.IsOK() && right.IsOK() )
	{
		...
		if ( left.IsNumber() && right.IsNumber() )
		{
			...
			{
				// Calculate the numerical value of the result by multiplying
				...
				else
				{
					...
					// MULPRECISION. Then, set the result type to FLOAT.
					result.SetValue( DoubleToStr( dlNumVal, MULPRECISION ) );
		...
		else
		{
			...
			// result to MUL_NO_NUM_ERROR.
			result.SetStatus( ExprResult::MUL_NO_NUM_ERROR );
		}
	}
	else
	{
