...
//						================
{
	...
	m_strVersion	= "6.1";
	m_strBuild		= "60";
	m_strCopyright	= "COPYRIGHT (C) 1999-2008 LOGICA NEDERLAND B.V. - ALL RIGHTS RESERVED";
}
...
)
{
	...
	if (ValidateEngineLicense(m_pSetting))
	{
...
)
{
	...
	if ( ValidateEngineLicense( pSetting ) )
	{
...
{	
	...
	if ( nCheckResult == 0 || nCheckResult == 1 )
	{
		...
		// Set the result of the function to the specified check result.
		bResult = bCheck;
	}
	...
	if (license.Import(strLicenseFile))
	{
		...
		if (key.Decrypt())
		{
			...
			else
			{
				...
				IOInterface::WriteIOError(string("Engine 6.1"), strError, string("Contact Logica for a valid license."));
			}
		}
		else
		{
			// Error, illegal key
			IOInterface::WriteIOError(string("Engine 6.1"), string("Engine license key is invalid."), string("Contact Logica for a valid license."));
		}
	}
	else
	{
			...
			if (key.CheckTimeLock())
			{
				...
				if (strKeyComponentName == strComponentName)
				{
					...
					else
					{
						...
						IOInterface::WriteIOError(string("TestFrame Engine 6.1"), strError, string("Contact Logica for a valid license."));
					}
				}
				else
				{
